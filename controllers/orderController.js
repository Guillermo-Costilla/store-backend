...
export const orderController = {
  ...
  async getUserOrders(req, res) {
    try {
      ...
      const result = await client.execute({
        sql: "SELECT * FROM ordenes WHERE usuario_id = ? ORDER BY fecha_creacion DESC LIMIT ? OFFSET ?",
        args: [usuario_id, Number.parseInt(limit), Number.parseInt(offset)],
      })
  ...
  async updateOrderStatus(req, res) {
    try {
      ...
      // Si se marca como enviado, reducir stock
      if (estado === "enviado") {
        const orderResult = await client.execute({
